...
namespace DVG.WIS.Utilities
{
    public class GoogleTOTP
    {
        ...
        private static readonly string secretKey = "DVG.WIS.TinXe";
        private static readonly string projectName = "DVG_TinXe";
        private byte[] randomBytes = Encoding.ASCII.GetBytes(secretKey);
        ...
        public bool GenerateImage()
        {
            ...
            try
            {
                var data = GenerateProvisioningImage(projectName, randomBytes, 500, 500);
                using (var imageStream = new MemoryStream(data))
                {
                    var bitmapQR = new Bitmap(imageStream);
                    bitmapQR.Save(HttpContext.Current.Server.MapPath("~/File/GGQR/QR.jpg"));
                }
                ret = true;
            }
            catch (Exception)
        ...
        {
            return GeneratePin(randomBytes, CurrentInterval);
        }
        ...
        {
            bool flag = false;
            if (code == GeneratePin(randomBytes, CurrentInterval))
            {
                flag = true;
            }
            return flag;
...
{
    public class GoogleAuthenValid : ValidationAttribute
    {
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            var gga = new DVG.WIS.Utilities.GoogleTOTP();
            if (value == null || !value.ToString().ToLower().Equals(gga.GeneratePin()))
            {
