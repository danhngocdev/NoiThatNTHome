...
namespace DVG.WIS.Utilities
{
    public class StringUtils
    {
        ...
        public static string AppendAdsToContent(string originalContent, string adsContent, int adsPosition = 5)
        {
            if (string.IsNullOrEmpty(originalContent)) return originalContent;

            if (string.IsNullOrEmpty(adsContent)) return originalContent;

            string newContent = Regex.Replace(originalContent, "><", "> <", RegexOptions.Singleline | RegexOptions.IgnoreCase);

            try
            {
                HtmlDocument htmlDoc = new HtmlDocument();

                adsContent = HttpUtility.HtmlDecode(adsContent);

                htmlDoc.LoadHtml(originalContent);

                HtmlNode appendNode = htmlDoc.CreateElement("div");

                appendNode.InnerHtml = adsContent;

                List<HtmlNode> nodeP = htmlDoc.DocumentNode.Elements("p").ToList();
                List<HtmlNode> nodeDiv = htmlDoc.DocumentNode.Elements("div").ToList();
                List<HtmlNode> nodeImage = htmlDoc.DocumentNode.Elements("img").ToList();

                bool insertabled = false;

                if (htmlDoc.DocumentNode != null && !string.IsNullOrEmpty(htmlDoc.DocumentNode.OuterHtml))
                {

                    if (nodeP.Any() && nodeP.Count() >= adsPosition)
                    {
                        int middlePosition = int.Parse(Math.Round(Decimal.Parse((nodeP.Count / 2).ToString(CultureInfo.InvariantCulture))).ToString(CultureInfo.InvariantCulture));

                        HtmlNode node = nodeP[middlePosition - 1];

                        htmlDoc.DocumentNode.InsertAfter(appendNode, node);

                        insertabled = true;
                    }
                    else if (nodeDiv.Any() && nodeDiv.Count() >= adsPosition)
                    {
                        int middlePosition = int.Parse(Math.Round(Decimal.Parse((nodeDiv.Count / 2).ToString(CultureInfo.InvariantCulture))).ToString(CultureInfo.InvariantCulture));

                        HtmlNode node = nodeDiv[middlePosition - 1];

                        htmlDoc.DocumentNode.InsertAfter(appendNode, node);

                        insertabled = true;
                    }
                    else if (nodeImage.Any() && nodeImage.Count() >= adsPosition)
                    {
                        int middlePosition = int.Parse(Math.Round(Decimal.Parse((nodeImage.Count / 2).ToString(CultureInfo.InvariantCulture))).ToString(CultureInfo.InvariantCulture));

                        HtmlNode node = nodeImage[middlePosition - 1];
        ...
        public static string UploadImageIncontent(string content, out string firstImage, string imageRootPath, string sourceDomain, string domainImage, Func<string, string, string, string, string> uploadImage)
        {
            firstImage = String.Empty;
            ...
            try
            {
                ...
                string strRegex = @"(?<BlockImage><img.+?src=[""'](?<SRC>.+?)[""'].*?>)";

                Regex myRegex = new Regex(strRegex, RegexOptions.IgnoreCase | RegexOptions.Singleline);

                MatchCollection matches = myRegex.Matches(newContent);

                if (matches != null)
                {
                    foreach (Match m in matches)
                    {
                        string blockImage = m.Groups["BlockImage"].Value;
                        string imageUrl = m.Groups["SRC"].Value;

                        string newImageUrl = uploadImage.Invoke(imageUrl, imageRootPath, sourceDomain, domainImage);

                        if (!string.IsNullOrEmpty(newImageUrl))
                        {
                            if (imageUrl == newImageUrl) continue;

                            firstImage = newImageUrl;
                            newContent = newContent.Replace(blockImage, string.Format("<p style=\"text-align:center\"><img src=\"{0}\" /></p>", newImageUrl));
                        }
                        else
                        {
                            newContent = newContent.Replace(blockImage, string.Empty);
                        }
                    }
                }
        ...
        public static string ConvertToMobileContent(string content, string domainImage, string resizeSize = "resize/480x-", string domain = "", string domainMobile = "")
        {
            string newContent = content;
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(domain) && !string.IsNullOrWhiteSpace(domainMobile))
                {
                    newContent = Regex.Replace(newContent, string.Concat("//", domain), string.Concat("//", domainMobile));
                }
            }
            catch (Exception ex)
            {
