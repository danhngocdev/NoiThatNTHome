...
namespace DVG.WIS.Core
{
    public static class CoreUtils
    {
        ...
        {
            if (String.IsNullOrEmpty(avatar)) return !String.IsNullOrEmpty(defaultAvatar) ? defaultAvatar : "/Content/images/noavatar.PNG";
            string domain = StaticVariable.DomainImage;
            return avatar.Contains(domain) ? avatar : domain.TrimEnd('/') + (avatar.StartsWith("/") ? avatar : "/" + avatar);
        }

        public static string BuildCropAvatar(string avatar, string defaultAvatar = "", string crop = "")
        {
            if (String.IsNullOrEmpty(avatar)) return !String.IsNullOrEmpty(defaultAvatar) ? defaultAvatar : "/Content/Web/images/noimage.16.9.jpg";

            string domain = StaticVariable.DomainImage;
            string avatarReturn = string.Empty;
            if (avatar.Contains(domain))
                avatarReturn = avatar.Replace(domain, domain.TrimEnd('/') + "/" + crop.TrimStart('/').TrimEnd('/'));
            else
                avatarReturn = domain.TrimEnd('/') + "/" + crop.TrimStart('/') + avatar;
            return avatarReturn;
        }
