...
$(function () {
    ...
    var ___isIE = /MSIE (\d+\.\d+);/.test(navigator.userAgent);
    if (___isIE) {
    ...
    $('.bootstrap-tagsinput input[type="text"]').autocomplete({
        ...
        select: function (event, ui) {
            if (ui.item != null) {
                $('.ui-autocomplete-input').val("");
                $('#txtAddTags').tagsinput('add', { Id: ui.item.tId, Name: ui.item.value, UnsignName: ui.item.label }, { preventPost: true });
            }
        }
    }).on('click', function () {
