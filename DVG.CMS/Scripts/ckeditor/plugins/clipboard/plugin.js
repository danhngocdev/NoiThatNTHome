...
//'use strict';
// upload image if html contain image
function doUploadOnPaste(html, callback) {
    //begin fix. Nếu chuột phải vào ảnh và copy Image thì chỉ có thẻ image thôi.
    try {
        // phải try catch nếu không khi chỉ copy text sẽ lỗi
        var $temp = $(html);
        if ($temp.is('img')) {
            html = '<p>' + html + '</p>';
        }
    } catch (e) { }
    CmsBm.Plugins.PhotoManager.DoUploadOnPaste(html, callback);
}
    ...
    CKEDITOR.plugins.add('clipboard', {
        ...
        init: function (editor) {
            ...
            editor.on('paste', function (evt) {
                var data = evt.data;
                if (data.type == 'html') {
                    if (editor.name == 'PhotoCMS_PhotoNameTxt') {
                        editor.insertText(removeAllHtmlTags(data.dataValue));
                    } else {
                        doUploadOnPaste(data.dataValue, function (html) {
                            data.dataValue = html;
                            var $temp = $('<div>' + html + '</div>');
                            //tupa added 20/11/2013 edit lỗi khi hover lên div và copy bị dính style trên chrome
                            $temp.find('span').css('background-color', 'transparent');
                            editor.insertHtml(CKEDITOR.removeUnnecessaryTags($temp.html()));
                            //editor.insertHtml($temp.html());
                        });
                    }
                } else {
