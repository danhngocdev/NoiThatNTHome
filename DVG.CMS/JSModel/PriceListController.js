(function () {
    app.controller('PriceListController', ['$scope', '$http', 'Service', '$sce', 'notify', '$window', '$ngConfirm', function ($scope, $http, Service, $sce, notify, $window, $ngConfirm) {
        ...
        $scope.InitCreateOrUpdate = function () {
            ...
            $scope.getPriceListById(0);
        };

        $scope.ListData = [];

        $scope.getPriceListById = function (Id) {
            Service.post("/PriceList/GetPriceListById", { Id: Id })
                .then(function (response) {
                    if (response.Success) {
                        if (response.Data) {
                            console.log(response.Data, 99);
                            $scope.PriceList = response.Data;
                            $scope.PriceList.ListStatus = response.Data.ListStatus;
                            $scope.PriceList.ListUnit = response.Data.ListUnit;
                            console.log(response.Data,99);
        ...
        $scope.getListData = function () {
            ...
            Service.post("/PriceList/Search", { searchModel: $scope.Search })
                .then(function (response) {
                    if (response.Success) {
            ...
            $ngConfirm({
                ...
                buttons: {
                    OK: {
                        ...
                        keys: ['enter'],
                        action: function (scope, button) {
                            Service.post("/PriceList/Delete", { id: id })
                                .then(function (response) {
                                    if (response.Success) {
                                        notify({
        ...
        $scope.updatePriceList = function () {
            Service.post("/PriceList/Update", { priceListModel: $scope.PriceList })
                .then(function (response) {
                    if (response.Success) {
                        if ($scope.PriceList.Id > 0) {
                            notify({
