(function () {
    app.controller('ProductShowHomeController', ['$scope', '$http', 'Service', '$sce', 'notify', '$window', '$ngConfirm', function ($scope, $http, Service, $sce, notify, $window, $ngConfirm) {
        ...
        $scope.getbyId = function (Id) {
            Service.post("/ProductShowHome/GetProductShowHomeById", { Id: Id })
                .then(function (response) {
                    if (response.Success) {
                        if (response.Data) {
                            console.log(response.Data, 99);
                            $scope.ProductShowHome = response.Data;
                            $scope.ProductShowHome.ListStatus = response.Data.ListStatus;
        ...
        $scope.getListData = function () {
            ...
            Service.post("/ProductShowHome/Search", { searchModel: $scope.Search })
                .then(function (response) {
                    if (response.Success) {
        ...
        //Delete PriceList
        $scope.deleteProductShowHome = function (id) {
            console.log(id);
            $ngConfirm({
                title: cms.message.confirmWarningTitle,
                ...
                buttons: {
                    OK: {
                        ...
                        keys: ['enter'],
                        action: function (scope, button) {
                            Service.post("/ProductShowHome/Delete", { id: id })
                                .then(function (response) {
                                    if (response.Success) {
                                        notify({
        ...
        $scope.updateProductShowHome = function () {
            Service.post("/ProductShowHome/Update", { productShowHome: $scope.ProductShowHome })
                .then(function (response) {
                    if (response.Success) {
                        if ($scope.ProductShowHome.Id > 0) {
                            notify({
