            ...
            switch (data.method) {
                ...
                case 'getCroppedCanvas':
                    if (result) {
                        ...
                        //}
                        fm.loading(true);
                        var uriBase64 = result.toDataURL(uploadedImageType);
                                ...
                                if (!result.OK) {
                                    ...
                                    fm.getImageInfoFromApi(result.OK, function (fileUrl, e) {
                                        fm.updateFileInfoToDb(fileUrl, e, function (d) {
                                            fm.load();
                                            $(fm.option.cropPanel).modal('hide');
                                            fm.loading(false);
                                        });
                                    })
                                }
                ...
                case 'getData':
                    var originalWidth = $('#cropPannelImage').attr('data-width');
                    var originalHeight = $('#cropPannelImage').attr('data-height');
                    var w = 0;
                    var h = 0;
                    if (dataRotate.value == 0 || dataRotate.value % 180 == 0) 
                    {
                    ...
                    var params = '{ "x": 0, "y": 0, "width": ' + w + ', "height": ' + h + ', "rotate": ' + dataRotate.value + ', "scaleX": 1, "scaleY": 1 }';
                    $('#putData').html(params);
                    $('#cropPannelSetDataButon').click();
                    break;
